...
- **In progress**: #613 part 1/3 (Read-only roles — Organization + Calendar pages: new `useModuleAccess(module)` hook (impersonation-aware), `ContextMenuItem.disabled`, `readOnlyStyle`, `common.read_only_hint`. Centers, Spaces, Activity Types (incl. schedule-rule buttons), Professional Services, Operating Hours (weekly grid + holidays), Staff (controls shown disabled instead of hidden; editor Save gated) and Calendar now disable write controls for roles without write access. New `read-only-writes.test.ts` (51 cases across MEMBERS/ORGANIZATION/TRAINING/NUTRITION/FINANCIALS/PAYMENTS). Client redirects stay until #611. Part 2 (Financials + Payments): Plans (incl. every in-row billing/centers/allowance/price edit button; *Assign to member* follows PAYMENTS, so front desk keeps it), Promotions, Sellable Items, Taxes, and the Assigned Plan row (status actions shown disabled instead of hidden; *Close* admin-only via the hook's new `isAdmin`). Transactions, Billing Events and Payment Providers have no write controls. Part 3 (Training + Nutrition) follows.)
