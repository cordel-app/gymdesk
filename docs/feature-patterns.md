...
- **A derived amount is resolved, not stored — and one module owns the resolution** (stage 15). `final_price` existed because "the agreed price after Promotions" felt like a fact worth writing down; it was a number with no date in it, so every rule that made the price depend on *when* (a free month, a benefit that ends with its Promotion) had to be duplicated somewhere or ignored. Once a stored amount and a resolver disagree, the stored one is what moves money and the resolver is what the customer was shown. Drop the column and give the resolution a single module that owns the SELECT list, the fallback rule, the "which cycle does *now* mean" rule and a batched variant, so a list page costs a bounded number of queries instead of one per row. The mutations that used to recompute the amount then write nothing: they resolve it before and after and record the *difference* on the ledger, which is the part that is genuinely history. Keep only the inputs the record itself owns (here `membership_fee_price`, its own regular fee) and give them exactly one writer.
