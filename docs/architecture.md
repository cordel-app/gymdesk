...
**Frontend**: #437 removed the standalone admin `[locale]/schedule/` page; session roster, attendance, shared-training requests, and the sharing toggle now open from `ClassSessionDetailPanel` on the Calendar. Remaining stage 4 work is collapsing the Calendar page's `_type`-branching into one detail panel.
