...
| Activity types | `activity-types.ts`, `activity-type-schedule-rules.ts` | `[locale]/activity-types/` | Admin-only CRUD (#70, redesigned #269). (#324) `is_shareable TINYINT DEFAULT 0` added — when true, this activity type's sessions can participate in shared-training bookings. `activity_types` table extended in #190 with `default_space_id`, `default_trainer_membership_id`, `color`; extended in #269 with `default_center_id FK→centers`, full audit columns (`created_by_membership_id`, `modified_at`, `modified_by_membership_id`, `deleted_at`, `deleted_by_membership_id`), and soft-delete. `gyms.timezone VARCHAR(64) DEFAULT 'Europe/Madrid'` added. New `activity_type_schedule_rules` table: one row per recurrence unit (type one_off/weekly/monthly; weekday 0=Sun…6=Sat; ordinal for monthly; start_time/end_time in HH:MM). Sub-router `/activity-types/:id/schedule-rules` (GET/POST/PUT/DELETE). `domain/scheduleEngine.ts`: Luxon-based occurrence generator + `materializeScheduleRule` (bulk-inserts calendar_events in chunks of 200) + `cancelFutureOccurrences` (preserves past, cancels future). `calendar_events.schedule_rule_id FK→activity_type_schedule_rules ON DELETE SET NULL`. `POST /:id/duplicate` deep-copies rules and materializes calendar events. `POST /:id/restore` restores soft-deleted types. `recycle-bin` extended with `activity_type` branch. Admin page: expandable-row UX — inline `+ Create Activity Type` row (Name/Duration/Capacity), collapsed header (Name/Description/Duration·Capacity/Default Center/Status/ContextMenu), expanded General + Schedule (inline rule CRUD) + Metadata sections; center→space linked dropdowns. **Member assignment on schedule rules (#366)**: new `activity_type_schedule_rule_members` table (`gym_id`, `schedule_rule_id` FK→`activity_type_schedule_rules` ON DELETE CASCADE, `member_id` FK→`members` ON DELETE CASCADE, unique per rule+member) — staff can pass `member_ids: number[]` on `POST`/`PUT .../schedule-rules[/:id]`; `PUT` with no `member_ids` key leaves the existing assignment untouched. `materializeScheduleRule` books every assigned Member (via `bookMemberOnSession(force=true)`, same mechanism as a manual staff add — capacity is a warning, never a hard block for staff) onto each freshly materialized `calendar_events` occurrence, so future occurrences generated by a later re-materialization (e.g. after `PUT`) keep receiving the same assignment. GET/POST/PUT responses include `member_ids`. Admin rule form: new `MemberMultiSelect` (`[locale]/calendar/MemberMultiSelect.tsx`, wraps `MemberSearchInput` with removable chips) shows a lightweight "N members assigned — capacity is X" warning above the configured `max_capacity`, never blocking. |
| Calendar events | `calendar-events.ts`, `calendar-event-series.ts` | `[locale]/calendar/` | TRAINING module (#190 #191). Standalone and recurring scheduled events. **Recurrence** (#191): `calendar_event_series` table stores the pattern template; `calendar_events` gains `series_id` FK (ON DELETE SET NULL) and `series_occurrence_date DATE` (canonical scheduled date, survives drag-and-drop). Pure-TS `domain/recurrenceEngine.ts` generates occurrence dates (daily/weekdays/weekends/weekly/monthly/yearly, configurable interval, end by date/count/never, 13-month rolling cap for never-ending series). "Exploded" storage: each occurrence is an independent `calendar_events` row — bookings and attendance attach to individual events. Scope-based mutations via `?scope=this|this_and_following|entire_series` on PUT/DELETE: `this` edits/cancels one; `this_and_following` splits the series (truncates original end_date, creates new series from cutDate); `entire_series` updates template + regenerates future occurrences. Recurring cancellation uses `status='cancelled'` (visible on calendar); standalone delete uses soft-delete. `GET /calendar-event-series` and `DELETE /calendar-event-series/:id` (cancels future + soft-deletes series row). MySQL DATE columns returned as JS Date objects by mysql2 — `toDateStr()` normalizer in calendar-events.ts handles both types. Admin UI: ScopeDialog appears before editing/deleting a recurring event; RecurrenceEditor component in CalendarEventModal for create and whole-series edits. Full CRUD with space/trainer conflict detection (409). FullCalendar v6 day/week/month views, drag-and-drop and resize (always `scope=this`). Soft-delete, full audit fields, status CHECK. **#326**: Calendar page now also fetches `class_sessions` (tagged `_type:'session'` in extendedProps, prefixed `cs-{id}`) and renders them alongside calendar events (prefixed `ce-{id}`). Clicking a class session opens `ClassSessionDetailPanel` (instance management panel) instead of `EventDetailsPanel`; sessions are non-draggable/non-resizable (time changes go through the panel's Change Time flow). `ClassSessionDetailPanel` provides: enrolled member list with Remove (→ `DELETE /bookings/:id`, atomic auto-promote), Add Member (member search → capacity check → `POST /bookings` with optional `force=true`), waiting list with Add to Waiting List (`POST /bookings` with `waitlist=true`), Change Time (`PUT /class-sessions/:id`), Cancel Event (`POST /class-sessions/:id/cancel`). `MemberSearchInput` debounced combo component uses `GET /members?q=`. `GET /class-sessions` accepts `activity_type_id`, `space_id`, `trainer_membership_id` filter params. `GET /members?q=` LIKE search on name/email, max 20 results. `POST /bookings` `waitlist=true` flag inserts as waitlisted regardless of capacity. |
| Class sessions | `class-sessions.ts` | `[locale]/schedule/` | Scheduled instances of an activity type (space, trainer, capacity). Create/update/cancel by admin/coach/staff. (#324) `allows_shared_booking TINYINT DEFAULT 0` — when true, a second group may book via a shared-training request. `PUT /:id` accepts `allows_shared_booking`. **Shared training capacity (#323)**: `spaces.max_concurrent_groups` and `gym_memberships.max_concurrent_groups` (TINYINT UNSIGNED DEFAULT 1) added. POST validates concurrent-group capacity when trainer+space are both set — atomically locks concurrent sessions via `SELECT FOR UPDATE`, rejects with 409 codes `slot_not_shareable` / `activity_not_shareable` / `slot_fully_occupied` / `sharing_not_authorized`; last code includes `host_session_id`. PUT revalidates on slot changes. New `PUT /:id/sharing-authorized {authorized: boolean}` (requireModuleWrite TRAINING) — disabling returns 409 if concurrent sessions exist. GET SELECT includes `is_shareable`, `allows_shared_booking`, `concurrent_groups_count`, `effective_max_groups` (LEAST of trainer+space limits). `effective_trainer_membership_id` records who actually delivered the session (set via `PUT /:id/effective-trainer`). `POST /:id/bulk-present` marks all pending confirmed bookings as present. `POST /:id/complete` — hard-blocked unless all confirmed bookings have a non-pending `attendance_status` and at least one trainer is set. GET SELECT includes `booked_count`, `attendance_present`, `attendance_absent`, `attendance_pending` subquery counts. **Member assignment on creation (#366)**: `POST /class-sessions` accepts optional `member_ids: number[]` — each is validated (gym-scoped, not soft-deleted) and booked via `bookMemberOnSession(force=true)` inside the same transaction as the session INSERT (atomic: an invalid member id fails the whole request before any row is created). Capacity is never a hard block for this staff-driven assignment; the response's existing `booked_count`/`effective_capacity` fields tell the caller when it's over. Admin Schedule page (`[locale]/schedule/page.tsx`) shows the `MemberMultiSelect` picker (create-flow only — editing an existing session's roster still goes through `ClassSessionDetailPanel`'s Add Member flow). |
| Bookings | `bookings.ts` | (inside Schedule) | Waitlist, capacity, attendance. `attendance_status VARCHAR(20)` ('pending'|'present'|'absent') tracks attendance separately from the booking lifecycle `status` ('booked'|'waitlisted'|'cancelled'). `POST /:id/attendance` sets attendance_status (staff-level: admin/front_desk/trainer roles). `POST /bookings` accepts `force=true` to add a walk-in over capacity (always inserts as 'booked', returns `over_capacity: true`). Credit consume/refund hooks via `package-credits` and `plan-allowances`. **#372 cancellation-timing refund rule**: `cancelBooking` only auto-refunds a spent package credit when the member cancels at least 1 day before the session (`DATEDIFF(session.starts_at, UTC_TIMESTAMP()) >= 1`); a same-day cancellation keeps the credit consumed by default. `POST /:id/refund-credit` (requireRole admin/front_desk/trainer_performance/trainer_perf_nutrition) lets staff explicitly refund a same-day-cancelled booking's credit — only works while `bookings.user_class_package_id` is still set (i.e. not already auto- or manually refunded), so a second call 400s instead of double-refunding. `class-sessions.ts`'s `POST /:id/walk-in` (same roles) books a member with no prior reservation and immediately marks them present in one transaction — reuses `bookMemberOnSession(force=true)` so the normal booking-time credit debit fires; rejects with 409 if the member already has an active booking for that session. |
