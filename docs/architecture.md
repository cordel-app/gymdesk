...
| Calendar Event Bookings | `event-bookings.ts` | (inside `[locale]/events/`, expandable roster rows) | Separate `event_bookings` table (not `bookings`). FIFO waitlist with auto-promote on cancel and on capacity increase (`promoteWaitlistOnCapacityIncrease` in `events.ts`). Admin: GET/POST/DELETE `/event-bookings`, POST `/:id/promote`. Uniqueness among non-cancelled (event_id, member_id) enforced at router level (MySQL has no partial unique indexes). Cancellation policy (member-facing): allowed if event starts >24 h from now OR booking made <4 h ago; waitlisted can always cancel. `GET /events` and `GET /events/:id` return `booked_count` + `waitlist_count`. |
| Member Notifications | — (member-facing via `/me/*`) | `[locale]/notifications/` | (#194) `member_notifications` table: gym_id, member_id, type, entity_type, entity_id, payload (JSON), read_at, created_at. 7 types: booking_confirmed, waitlist_joined, promoted_from_waitlist, event_cancelled, event_updated, booking_reminder_24h, booking_reminder_1h. Written fire-and-forget by `sendNotification`/`sendBulkNotification` in `infra/notifications.ts` — never blocks the response. Session cancellation bulk-notifies all booked members. `cancelBooking`/`cancelEventBooking` now return `promotedMemberId` so the caller can notify promoted members. |
...
| Promotions | `promotions.ts`, `promotion-details.ts` | `[locale]/promotions/` | Admin-only. Plan targeting, charge benefits, period benefits. Soft delete via `lifecycle_status='deleted'` + `deleted_at` + `deleted_by_membership_id` (migration 093). Free period config: `free_period_paid_months` + `free_period_bonus_months` on the `promotions` table. `promotion_charge_benefits` uses `gym_charge_id FK→gym_charges` + `action VARCHAR(30)`; replace-all `PUT /charge-benefits`. `promotion_period_benefits` redesigned (migration 093): `charge_type_id FK→charge_types`, `quantity`, `frequency_interval`, `frequency_unit ('week'\|'month')`, `enabled`; replace-all `PUT /period-benefits`. New charge_type seeds (migration 093): `nutrition_service`, `personal_training`, `duet_personal_training`, `group_class` (all `is_gym_charge=0`). Admin page: expandable-row list, temp-row creation (no DB write until Save), Edit via context menu only, Save writes promo + all sub-resources atomically. `action_types` no longer referenced by promotions. |
...
| `/schedule` | `GET /me/schedule`, `POST /me/bookings`, `DELETE /me/bookings/:id`, `GET /me/bookings`, `GET /me/events`, `POST /me/event-bookings`, `DELETE /me/event-bookings/:id` | Browse sessions and Calendar Events unified by day; book, cancel, view bookings. Cancellation policy enforced on `/me/event-bookings/:id` DELETE. `GET /me/schedule` joins `activity_types`+`spaces` (not class_types+rooms) and returns `space_name`, `trainer_name`, `can_cancel`. |
...
| `/upcoming` | `GET /me/upcoming` | Next 30 days of confirmed bookings (sessions + events) merged and sorted by starts_at. |
| `/activity-history` | `GET /me/activity-history` | Past bookings (sessions + events) paginated, newest first. |
