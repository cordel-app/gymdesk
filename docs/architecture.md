...
- **Frontend**: `apps/member/src/context/FeatureFlagsContext.tsx` mirrors the admin app's context (`GET /feature-flags`, no gym scoping) and exports `isFeatureEnabled(flags, key)` (ancestor-cascade lookup, same semantics as the backend's `requireFeatureEnabled`, defaults to enabled when a key has no row). Home (`page.tsx`) hides the My Training Plan / My Bookings / My Nutrition nav tiles, the Next Booking section, and the My Membership summary card when disabled (Calendar's tile is never gated — it isn't part of this flag set). Each of `/training`, `/nutrition`, `/schedule`, `/membership`, `/profile` redirects to Home in its existing `appLoading`/`isLinked` bootstrap effect when its flag is disabled and the caller isn't a superadmin. **(#438)** The Next Booking section explicitly gates on `member_web.my_bookings` in the frontend even though the `GET /me/schedule` call backing it is intentionally ungated at the API layer (see the shared-endpoint exception above) — without this, disabling the flag left the widget (and its `DELETE /me/bookings/:id` cancel button, which *is* backend-gated) visibly stuck rather than hidden. `FeatureFlagsProvider` also polls `GET /feature-flags` every 30s (matching the backend's in-memory cache TTL) instead of fetching once on mount, so an admin's toggle reaches an already-open member session without a full reload.
