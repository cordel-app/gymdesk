...
| Calendar events | `calendar-events.ts` | `[locale]/calendar/` | TRAINING module (#190). Standalone scheduled events (not class sessions). Full CRUD with space/trainer conflict detection (409 overlap check: `starts_at < new_ends_at AND ends_at > new_starts_at`, cancelled events excluded). FullCalendar v6 (dayGrid/timeGrid/interaction plugins): day/week/month views, drag-and-drop and resize via PUT. Filter bar: All / Space / Activity Type / Trainer pill modes with a dropdown when a mode is selected. `CalendarEventModal` auto-populates color, default_space, default_trainer, and recalculates `ends_at` from `duration_minutes` when an activity type is selected. Soft-delete (`deleted_at`), full audit fields, status CHECK (`draft/scheduled/completed/cancelled`). `/events` admin page removed. |
...
| `/schedule` | `GET /me/schedule`, `POST /me/bookings`, `DELETE /me/bookings/:id`, `GET /me/bookings`, `GET /me/events`, `POST /me/event-bookings`, `DELETE /me/event-bookings/:id` | Browse sessions and Calendar Events unified by day; book, cancel, view bookings. Cancellation policy enforced on `/me/event-bookings/:id` DELETE. |
