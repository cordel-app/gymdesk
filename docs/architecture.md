...
- **Impersonation precedence (#439)**: the frontend's Super Admin bypass (`isSuperadmin || isFeatureEnabled(...)`) only applies when the caller is acting in their *native* capacity. Home and each of `/training`, `/nutrition`, `/schedule`, `/membership`, `/profile` now also read `useImpersonation().isImpersonating` and gate the bypass on `isSuperadmin && !isImpersonating`, so a superadmin impersonating a Member sees exactly that Member's `member_web.*` visibility instead of always seeing everything. Two prerequisite bugs made this invisible until now: (1) both `FeatureFlagsContext`s (admin + member) called `/feature-flags` without a Clerk token, so the request 401'd and flags silently stayed `{}` — fixed unrelated to #439 (see `docs/roadmap.md`); (2) `AppContext.isSuperadmin` is sourced from the *logged-in* Clerk user's `publicMetadata`, which does not change during impersonation — this is intentionally left as-is (`AdminBar`/`GymSwitcher` need the real superadmin flag to keep their own chrome working), so the fix combines it with `isImpersonating` at each feature-flag-bypass call site instead of changing what `isSuperadmin` means. The equivalent bug existed in `apps/admin/src/components/Sidebar.tsx`'s `userRole` (`isSuperadmin ? 'superadmin' : activeGym?.role`, feeding `filterNavGroups()`'s combined role+feature-flag nav filtering) and got the same fix. The backend `requireFeatureEnabled` bypass described above is deliberately untouched — impersonation remains a superadmin support/debug capability with full underlying access; only the on-screen UI now mirrors the impersonated user's view.
