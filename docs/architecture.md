...
| Promotions | `promotions.ts`, `promotion-details.ts`, `action-types.ts` | `[locale]/promotions/` | Admin-only. Plan targeting, charge benefits, period benefits. `action_types` is a global lookup used by period benefits. `promotion_charge_benefits` now uses `gym_charge_id FK→gym_charges` + `action VARCHAR(30)` (migration 092 replaced old `charge_type_id` + `action_type_id`; UNIQUE on `(promotion_id, gym_charge_id)`). Charge benefits endpoint is replace-all `PUT /charge-benefits`; old per-row POST/PUT/:id/DELETE endpoints removed. List and detail GET queries read `pcb.action` directly — no `action_types` join needed for charge benefits. |
