...
export function PlanWorkoutBlocksModal({ memberId, planId, workoutId, workoutName, onClose }: {
  memberId: number; planId: number; workoutId: number; workoutName: string; onClose: () => void;
}) {
  ...
  const base = `/members/${memberId}/training-plans/${planId}/workouts/${workoutId}/blocks`;

  async function load() {
    setLoading(true);
    try {
      const plan = await apiFetch<{ workouts: { id: number; blocks: Block[] | null }[] | null }>(`/members/${memberId}/training-plans/${planId}`);
  ...
  useEffect(() => { load(); }, [memberId, planId, workoutId]);
  ...
  async function move(idx: number, dir: -1 | 1) {
    ...
    try { await apiFetch(`${base}/reorder`, { method: 'PUT', body: JSON.stringify({ order: order.map((b) => b.id) }) }); load(); }
    catch (err: any) { toast(err.message ?? t('workout_template_blocks.error_generic')); }
  }
  ...
  return (
    <div style={overlayStyle} onClick={onClose}>
      <div style={{ ...modalStyle, width: 720 }} onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ margin: '0 0 20px', color: '#666', fontSize: 14 }}>{workoutName}</p>
        ...
        {exercisesFor && (
          <PlanBlockExercisesModal
            memberId={memberId} planId={planId} workoutId={workoutId} blockId={exercisesFor.id}
