...
Work the open PRs that target the main branch oldest-first, starting with the oldest. A PR you cannot finish never ends the run — you move on to the next one, so one stuck PR cannot hold up the ones behind it.
...
- If CI is still red after a reasonable fix attempt, or the failure needs a judgment call outside the ticket's scope, do not merge — leave a PR comment explaining what's failing and why, then **continue to the next-oldest open PR instead of ending the run**. A blocked PR must never hold up the queue behind it: a green, mergeable PR further down still gets reviewed and merged in the same run. Only stop once every open PR has been either merged or accounted for.
- Before leaving that comment, check whether a previous run already commented on the same failure on the same PR. If it did and nothing has changed since — same head commit, same failing check — do not post a second comment repeating it; just move on to the next PR. An unanswered comment restated every run adds noise, not information. Comment again only when something actually changed: a new head commit, a different failure, or new information worth having.
