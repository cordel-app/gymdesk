...
- Every new code path that inserts into `gyms` must set `payment_provider_id` (NOT NULL since migration 174). Take the catalogue's default (`is_default = 1 AND deleted_at IS NULL`) unless the caller names one — `api/src/api/gyms.ts`, `api/src/test/helpers.ts` and `api/src/infra/seed.ts` are the three existing paths.
