...
- The **nightly billing run** (`POST /billing/run`) prices the cycle `next_billing_date` names, never "today" and never `final_price` alone: `priceDueMembershipFee()` (`api/src/api/billing-run-pricing.ts`) asks the Billing Simulation's own `resolveMembershipFee()` whether that date is waived, so what is charged cannot drift from what the staff screens and the Member's My Membership page show (#635 stage 11). A waived cycle calls no payment provider and writes no `payment_requests` row — it records a `waived_billing` billing event and advances `next_billing_date`, leaving `last_billed_at` untouched. A new `billing_events.event_type` goes in **two** places: the writer and the `billing_events_event_type_check` CHECK (current definition: migration 185) — adding only the first makes the INSERT fail and takes the run's transaction with it.
- There is exactly **one** implementation of "what does the Membership Fee cost on this date": `resolveMembershipFee()` (`api/src/domain/billingSimulation.ts`), and every path calls it — the Billing Simulation, the Billing Events projection (`computeMembershipFeePriceAt()`), `computeFinalPrice()`, `GET /me/membership`'s `upcoming_payments` and the nightly run (#635 stage 12). A second copy of the arithmetic is what stage 12 existed to remove: the Member was shown one price and charged another for the same cycle, in both directions. An applied Promotion's **Membership Fee Benefit lives inside the Promotion's own Free/Paid/Bonus timeline and ends with it** (the thread's stage 12 answer (a)) — a Promotion configured with no months discounts nothing at all, and a benefit whose own `duration_months` outlasts the Promotion is capped at it (`effectiveBenefitDurationMonths`, #625). `final_price` is a stored number with no date in it, so it is never the answer for a *future* cycle: price the cycle. The number every path discounts *from* is `regularMembershipFee()` (`api/src/api/user-memberships.ts`) — the assignment's frozen `membership_fee_price`, then its Plan's price window, then a non-zero `base_price`, then `final_price` as the last resort; `base_price` alone is a constant 0 since migration 058, and `final_price` already has the applied Promotions baked in. The corrected pricing moves money, so the two writers that do (`computeFinalPrice()` and `POST /billing/run`) are gated on `billing.date_aware_membership_fee` (seeded **off**, migration 186, whose `down()` keeps the row because a missing key reads as *enabled*); read-only surfaces resolve dates through the rule regardless. Until that flag is switched on, `GET /user-memberships/reports/membership-fee-drift` and the run's `drift` counter report what it would change — do not make a money-moving pricing correction land without that kind of report first.
