...
async function createUserMembershipDirect(
  ...
  const { insertId } = await db.query(
    `INSERT INTO user_memberships (gym_id, member_id, membership_plan_id, status, starts_at, membership_fee_price)
...
async function createUserMembershipWithPrice(
  ...
  const { insertId } = await db.query(
    `INSERT INTO user_memberships (gym_id, member_id, membership_plan_id, status, starts_at, ends_at, base_price, membership_fee_price)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
describe('GET /user-memberships — lifecycle_status', () => {
  ...
  it("reports 'expired' for an active row whose ends_at has passed", async () => {
    ...
    const planId = await createPlan(gymId);
    const { insertId: umId } = await db.query(
      `INSERT INTO user_memberships (gym_id, member_id, membership_plan_id, status, starts_at, ends_at, membership_fee_price)
...
describe('POST /user-memberships/:id/assign-new-plan', () => {
  ...
  it('accepts a membership_fee_price override together with a discount_reason', async () => {
    ...
    expect(res.status).toBe(201);
    // #635 stage 15 — the negotiated price *is* the new assignment's own regular
    // fee, and `membership_fee` is what that comes to on its first cycle.
