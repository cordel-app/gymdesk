...
describe('GET /me/membership — upcoming payments are priced per cycle (#635 stage 12)', () => {
  ...
  beforeAll(async () => {
    ...
    // January to March 2099 — so the March cycle is still discounted and the
    // April one is not. #635 stage 15: there is no stored discounted number to
    // disagree with that, only the regular fee and the application.
    const { insertId } = await db.query(
      `INSERT INTO user_memberships
         (gym_id, member_id, membership_plan_id, status, starts_at, base_price,
          next_billing_date, recurring_billing_interval, recurring_billing_unit, membership_fee_price)
  ...
  it('shows the discounted cycle and the regular one that follows it', async () => {
    const { body } = await getMembership(gymId);
    // Until stage 12 this repeated one stored number (48.00) for every future
    // date — a promise the nightly run would not keep once the Promotion's own
    // months were over.
  ...
  it('resolves the same amounts the nightly run would charge for those dates', async () => {
    ...
    const march = await priceMembershipFeeOn(rows[0] as any, NEXT_BILLING);
    const april = await priceMembershipFeeOn(rows[0] as any, '2099-04-10');
    expect([march.amount, april.amount]).toEqual([48, 60]);
  });

  // #635 stage 15 — the page states what the Member pays for their current cycle
  // as a resolved number. It used to be the stored `final_price`, which is what
  // let the page and the nightly run disagree.
  it('reports membership_fee for the current cycle, and no stored final_price', async () => {
    const { body } = await getMembership(gymId);
    expect(Number(body.membership.membership_fee)).toBe(48);
    expect(body.membership).not.toHaveProperty('final_price');
  });
