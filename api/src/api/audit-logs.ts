 ...
 * Server-side pagination with filters on entity_type, entity_id, entity_name,
 * actor, action, source, and date range.
 *
...
auditLogsRouter.get('/', requireRole('admin'), async (req, res) => {
  ...
  // #642: exact-match on the entity's own id, so a caller (e.g. the Member Details
  // modal) can deep-link to one record's history without relying on its name.
