...
userMembershipsRouter.delete('/:id', requireRole('admin'), async (req, res) => {
  const { gymId } = getTenantContext(req);
  const { rowCount } = await db.query(
    "UPDATE user_memberships SET status = 'cancelled' WHERE id = ? AND gym_id = ? AND status <> 'cancelled'",
    [req.params.id, gymId],
  );
  if (rowCount === 0) return res.status(404).json({ error: 'Membership not found or already cancelled' });
  res.status(204).send();
});
