...
import {
  ...
  loadMembershipFeeResolver,
  nextPricingDate,
  priceMembershipFeeNow,
...
// Membership Fee resolver compares dates as strings.
function toDateOnly(v: unknown): string {
  return v instanceof Date ? v.toISOString().slice(0, 10) : String(v).slice(0, 10);
}

/**
 * The Member's next two scheduled charges.
 *
 * #635 stage 12 — `priceOn` prices each date instead of repeating one stored
 ...
 * nothing. Stage 15 made it the only way to price one of these dates — the stored
 * `final_price` this used to fall back to is gone, and with it the disagreement
 * between what a Member is shown and what the nightly run charges.
 */
export function computeUpcomingPayments(
  ...
  if (!nextBillingDate || !billingInterval || !billingUnit) return [];
...
async function createSelfRegisteredMember(
  ...
  try {
    ...
    const { rows } = await db.query('SELECT * FROM members WHERE id = ? AND gym_id = ?', [insertId, gymId]);
    return rows[0];
  } catch (err: any) {
...
meLinkRouter.post('/', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (isSelfRegistration) {
      if (await isStaffLoginEmail(gymId, email)) return res.status(409).json({ error: STAFF_EMAIL_CONFLICT });
      const created = await createSelfRegisteredMember(gymId, userId, email, signup!, clerkUser);
      if (!created) {
        // A retry after a successful link lands here: same account, same gym.
        const { rows: linked } = await db.query(
          'SELECT * FROM members WHERE clerk_user_id = ? AND gym_id = ? AND deleted_at IS NULL',
          [userId, gymId],
        );
        if (linked[0]) return res.json(linked[0]);
  ...
  try {
    ...
    const { rows } = await db.query(
      ...
      `SELECT ce.id, ce.activity_type_id, ce.starts_at, ce.ends_at,
              ...
              CASE
                ...
                THEN (
                  ...
                  WHERE ce2.gym_id = ce.gym_id AND ce2.activity_type_id IS NOT NULL
                    ...
                    AND ce2.status <> 'cancelled' AND ce2.deleted_at IS NULL
                )
                ELSE 1
              ...
              (
                SELECT ceb.waitlist_position FROM calendar_event_bookings ceb
                WHERE ceb.calendar_event_id = ce.id AND ceb.member_id = ? AND ceb.status = 'waitlisted'
                LIMIT 1
              ) AS my_waitlist_position,
              (
                SELECT ceb.id FROM calendar_event_bookings ceb
                WHERE ceb.calendar_event_id = ce.id AND ceb.member_id = ? AND ceb.status <> 'cancelled'
                LIMIT 1
              ) AS my_booking_id,
              (
  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT ceb.id, ceb.calendar_event_id AS class_session_id, ce.starts_at,
       ...
       JOIN calendar_events ce ON ce.id = ceb.calendar_event_id
       JOIN activity_types at ON at.id = ce.activity_type_id
       WHERE ceb.id = ? AND ceb.gym_id = ? AND ceb.member_id = ?`,
...
meRouter.get('/shared-training-requests', requireRole('member'), requireFeatureEnabled('calendar.member_calendar'), async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const memberId = await resolveMemberId(gymId, ctx);
    const { rows } = await db.query(
      `SELECT str.*, str.calendar_event_id AS class_session_id,
  ...
  try {
    ...
    const { rows: mships } = await db.query(
      `SELECT ${MEMBERSHIP_FEE_COLUMNS},
              um.member_id, um.discount_reason, um.discount_expires_at,
              um.ends_at, um.status, um.created_at, um.next_billing_date,
    ...
    // #635 stage 12 — each upcoming charge is priced on its own date by the same
    // resolver the nightly run uses, so what this page promises the Member is
    // what the run will take: a cycle inside the contract's Free Period or Bonus
    // Duration shows €0, and an applied Promotion's Membership Fee Benefit stops
    // discounting once the Promotion's own Free/Paid/Bonus timeline ends.
    const { priceOn } = await loadMembershipFeeResolver(gymId, um);
    const upcoming_payments = computeUpcomingPayments(
      um.next_billing_date,
      um.billing_interval,
      um.billing_unit,
      (date) => priceOn(date).amount,
    );
    // #635 stage 15 — what the Member owes on the cycle they are next charged for,
    // resolved rather than read from a stored `final_price`: the column carried no
    // date, so this page used to promise a promotional price for a cycle after the
    // Promotion's months had run out — and the nightly run charged something else.
    const membership_fee = priceOn(nextPricingDate(um)).amount;
...
meRouter.post('/payment-requests', requireRole('member'), memberPaymentRateLimit as any, async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const { insertId } = await db.query(
      ...
      [gymId, um.id, memberId, fee, ctRows[0].id, orderId, result.providerOrderId, pageToken, pageTokenExpires],
    );
...
meRouter.get('/upcoming', requireRole('member'), requireFeatureEnabled('member_web.my_bookings'), async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const { centerId, allowedCenterIds } = getCenterContext(req);
    const where: string[] = [
      'ceb.gym_id = ?', 'ceb.member_id = ?', "ceb.status = 'booked'",
      "ce.status = 'scheduled'", 'ce.starts_at >= ?', 'ce.starts_at <= ?',
    ];
