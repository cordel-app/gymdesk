 ...
 * on our side of the boundary — `user_memberships.final_price`,
 * `billing_events.amount`, `payment_requests.amount`, what `resolveMembershipFee()`
 * returns — is a decimal number of euros. Every caller of `createPaymentRequest()`
 * and `executeRecurring()` converts through this helper, so a customer-initiated
